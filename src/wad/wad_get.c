...
/*   Updated: 2020/10/09 18:56:58 by bdrinkin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			wad_get_nodes(t_doom_nukem *doom, char *map_name)
{
	...
	while (offset < temp_offset)
	{
		...
		doom->wad.nodes[i].right_baby = bytes_to_short(doom->wad.map, offset + 24);
		doom->wad.nodes[i].left_baby = bytes_to_short(doom->wad.map, offset + 26);
		offset += 28;
		i++;
	}
	// i = 0;
