...
/*   Updated: 2020/08/29 20:55:37 by bdrinkin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_frames			*init_editor(t_doom_nukem *doom)
{
	...
	frame_table = new_frame(&q, 0xffffff, NULL);
	canvas.x = frame_table->main_frame->w;
	canvas.y = 0;
	temp = frame_table;
	q = oblast(frame_table->main_frame->w * 4, 0, frame_table->main_frame->w, frame_table->main_frame->h);
	frame_table->next = new_frame(&q, 0xffffff, NULL);
	frame_table = frame_table->next;
	q = oblast(frame_table->main_frame->w + 1, frame_table->main_frame->h / 5 * 4, doom->sdl.width / 5 * 3, frame_table->main_frame->h / 5);
